        ...
        sequelize.query(
            ...
            sequelize.query(
                ...
                foreignKeys.forEach(fk => {
                    associations+= `
                    ${request.tableName.charAt(0).toUpperCase() + request.tableName.slice(1)}.belongsTo(${fk.REFERENCED_TABLE_NAME.charAt(0).toUpperCase() + fk.REFERENCED_TABLE_NAME.slice(1)}, {foreignKey: '${fk.COLUMN_NAME}', as: '${fk.COLUMN_NAME.slice(fk.COLUMN_NAME.length - 2)}'})`
                })
                let functionRequest = {};
